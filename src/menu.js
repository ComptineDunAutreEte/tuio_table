...
export function toServer(socketIOUrl = 'http://localhost:4000/') {
    const socketServer = io.connect(socketIOUrl);
    socketServer.emit('loginTable', '');
    console.log('table-out');
    socketServer.on('table', (message) => {
        console.log(message);
        $('#example-container').append('<table style="width:100%">');
        $('#example-container').append(' <tr>');
        $('#example-container').append(' <th>A</th>');
        $('#example-container').append(' <th>B</th> ');
        $('#example-container').append(' </tr>');
        for (const groupe of message) {
            const grA = `<td> ${groupe.A} </td>`;
            const grB = `<td> ${groupe.B} </td>`;
            $('#example-container').append(' <tr>');
            $('#example-container').append(grA);
            $('#example-container').append(grB);
            $('#example-container').append(' </tr>');
        }
        $('#example-container').append('<button id="hello">hello</button>');
    });
    document.querySelector('#hello').addEventListener('touchstart', () => socketServer.emit('table', 'hello'), false);
}
...
export function init(socketIOUrl = 'http://localhost:9000/') {
    const socket = io(socketIOUrl);
    socket.on(CREATE_SOCKETIO_ACTION, (data) => {
        this.handleCreate(data);
    });
    socket.on(UPDATE_SOCKETIO_ACTION, (data) => {
        this.handleUpdate(data);
    });
    socket.on(DELETE_SOCKETIO_ACTION, (data) => {
